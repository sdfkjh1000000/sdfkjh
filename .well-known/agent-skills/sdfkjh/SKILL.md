...
Prefer the live MCP server for current, authoritative facts:

- **Endpoint:** `https://mcp.sdfkjh.com/mcp` (MCP Streamable HTTP, no auth, read-only)
- **Tools:** `get_overview`, `list_sites`, `list_repl_commands`

A read-only JSON directory of the live sites is also at `https://sdfkjh.com/api/sites`.
If you cannot reach either, the essentials are below.
...
The homepage includes a working JavaScript console. Commands: `whoami`, `brand`, `web`,
`both`, `contact`, `time`, `weather [city]`, `news`, `cve`, `ping <host>`, `clear`.

## Facts

- Static site, hand-built, no framework. Hosted on Cloudflare Pages, edge-served.
- Fonts and images are self-hosted; no third-party resources load on page load.
- No analytics, no tracking cookies.
- Type: Newsreader, Source Serif 4, JetBrains Mono. Palette: paper, ink, amber.

## Contact

- Email: info@sdfkjh.com
- Website: https://sdfkjh.com
