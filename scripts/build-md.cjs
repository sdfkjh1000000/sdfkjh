...
//
// index.html is skipped on purpose: the homepage is an interactive showcase
// (REPL, design editor, effects lab) that does not turn down to clean prose.
// index.md is hand-authored to represent the page faithfully — keep it in sync by hand.
const fs = require("fs");
...
const SITE = "https://sdfkjh.com";

const SKIP_DIRS = new Set(["node_modules", "scripts", "functions", "assets", "brand", "variants", "dist", ".git", ".claude"]);
...
function convert(rel) {
  ...
  const title = (doc.querySelector("title")?.textContent || "sdfkjh").trim();
...
console.log(`Generated ${results.length} markdown files. (index.md is hand-authored — not regenerated here)`);
